...
const NAMES: &[&str] = &[
    "dhodo", "zan", "ian", "firman", "gilang", "kurawal", "reyner", "fakhri",
];
...
pub const MENU_OPTIONS: [&str; 6] = [
    " Commit ", " Edit  ", " Retry ", " Lang  ", "Manual ", " Quit  ",
];
...
// wrap satu paragraf jadi baris-baris selebar `w` cell (display-width, bukan byte),
// tanpa memotong isi — teks yang kepanjangan lanjut ke baris berikutnya
fn wrap_paragraph(p: &str, w: usize) -> Vec<String> {
    let mut out = Vec::new();
    if w == 0 {
...
// wrap pesan utuh jadi baris display: baris bullet "- " memakai hanging indent
// (lanjutan teks rata setelah "- ", bukan jatuh ke kolom 0), sisanya wrap biasa
pub fn wrap_display(msg: &str, w: usize) -> Vec<String> {
    let mut out = Vec::new();
    for paragraph in msg.split('\n') {
        ...
        if let Some(rest) = paragraph.strip_prefix("- ") {
            let pieces = wrap_paragraph(rest, w.saturating_sub(2));
            if pieces.is_empty() {
                out.push("- ".to_string());
            } else {
                for (i, piece) in pieces.iter().enumerate() {
                    out.push(format!("{}{}", if i == 0 { "- " } else { "  " }, piece));
                }
            }
        } else {
            out.extend(wrap_paragraph(paragraph, w));
        }
    }
    out
}
...
fn msg_lines(msg: &str, w: u16) -> u16 {
    ...
    wrap_display(msg, w as usize).len().max(1) as u16
}
    ...
    terminal.draw(|f| {
        ...
        match &app.state {
            ...
            AppState::Loading(txt) => {
                ...
                let msg_h =
                    (msg_lines(&app.commit_msg, content_w) + 2).min(area.height.saturating_sub(3));
            ...
            _ => {
                ...
                let content_w = box_w.saturating_sub(2);
                let msg_h =
                    (msg_lines(&app.commit_msg, content_w) + 2).min(area.height.saturating_sub(6));
...
fn render_header(f: &mut ratatui::Frame, area: Rect, app: &App) {
    ...
    let spans = vec![
        ...
        Span::styled(
            format!("{}{}", " ".repeat(max_name - name_text.len()), model_text),
            Style::default()
                .fg(Color::Cyan)
                .add_modifier(Modifier::BOLD),
        ),
...
fn render_message(f: &mut ratatui::Frame, area: Rect, msg: &str) {
    ...
    // message content — wrap display-width (bullet hanging indent), tanpa truncate
    let lines: Vec<ratatui::text::Line> = wrap_display(msg, cw)
        .into_iter()
...
fn justified_line(items: &[String], w: usize) -> String {
    ...
    let (base, mut rem) = if total < w {
        ((w - total) / gaps, (w - total) % gaps)
    } else {
        (1, 0)
    };
    let mut out = String::new();
...
fn render_loading(f: &mut ratatui::Frame, msg_area: Rect, txt: &str, frame: u32) {
    ...
    let box_area = Rect {
        x: msg_area.x,
        y: msg_area.y + msg_area.height.saturating_sub(h),
